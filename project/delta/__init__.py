...
from os import path
from arpeggio import visit_parse_tree, NoMatch #type: ignore
from arpeggio.cleanpeg import ParserPEG #type: ignore
...
class Compiler:
    ...
    def realize(self,
                ...
                wat_file='web/output.wat'):

        def create_parser():
            with open(self.__peg_file) as file:
                grammar = file.read()
            return ParserPEG(grammar, self.__root_rule, 'comment')
        ...
        def generate_wat_code():
            ...
            wat_code = visit_parse_tree(self.__parse_tree, code_visitor)
            if wat_file:
